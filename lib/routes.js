...
module.exports = function (app) {
   ...
   app.post('/register', async function (req, res) {
      try {
         ...
         console.log(sc.decrypt(encryptedPassword));
      ...
      try {
         ...
         var currentAccountsData = await JSON.stringify(client.query('SELECT id, "first_name", "email", "password" FROM "users" WHERE "email"=$1', [username], function(err, result) {
            ...
            if (result.rows[0] == null) {
               ...
               passwordCompare(password, result.rows[0].password, function(same) { /// NEED TO REMOVE BCRYPT OR GET IT TO WORK
                  if (same) {
